...
export class gcLog extends plugin {
  ...
  async exportLog() {
    ...
    if (this.e.msg.includes('json')) {
      ...
      exportLog = await exportLog.exportJson()
    } else {
      // return await exportLog.exportXlsx()
      exportLog = await exportLog.exportXlsx()
    }
    if (exportLog != false) {

      if (this.e.isGroup) {
        await this.e.group.fs.upload(exportLog).catch((err) => {
          logger.error(`${this.e.logFnc} 发送文件失败 ${JSON.stringify(err)}`)
        })
      } else {
        await this.e.friend.sendFile(exportLog).catch((err) => {
          logger.error(`${this.e.logFnc} 发送文件失败 ${JSON.stringify(err)}`)
        })
      }
