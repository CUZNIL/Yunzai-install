...
1. **32位系统** ：该教程为了简化步骤直接使用时雨脚本，该脚本不支持32位系统。

2. **空闲存储空间小于32G** ：云崽本体用不到那么大容量，但是为了你愉快的下载bot需要的资源以及不出一些意料之外的问题，建议保证容量充足！不足也可以尝试，不保证成功。

3. **MIUI14** ：MIUI14使用安卓13，锅巴插件会报错找不到系统信息，其他插件也可能有类似问题，所以不建议使用。如需装载bot，建议先将机子降级为MIUI13，以免后续bug难以处理。尚不方便验证其他系手机的安卓13会不会有类似问题。
